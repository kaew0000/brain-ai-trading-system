...
class FeatureStore:
    ...
    def update_outcome(self, row_id: int, result: float | None,
                        pnl: float | None, holding_time_s: float | None) -> bool:
        with self._conn() as c:
            cur = c.execute(
    ...
    def update_outcome_by_trade_id(self, trade_id: int, result: float | None,
                                    pnl: float | None, holding_time_s: float | None) -> bool:
        with self._conn() as c:
            cur = c.execute(
    ...
    def get_recent(self, limit: int = 100, symbol: str | None = None) -> list[dict]:
        with self._conn() as c:
            if symbol:
    ...
    def get_training_rows(self, limit: int = 10_000, symbol: str | None = None) -> list[dict]:
        with self._conn() as c:
            if symbol:
                rows = c.execute(
